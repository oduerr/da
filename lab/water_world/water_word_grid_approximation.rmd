...
un_norm_post = function (p){
  return (dbinom(W, size = N, prob = p) * dunif(p, 0, 1))
}
...
c) Draw 10 samples from the posterior in b), use the function `sample` make use of the `prob` argument and take care of the argument `replace`. 

```{r, echo=lsg, eval=lsg, fig.height=4}
  sample(ps, 10, prob = post_b, replace=TRUE)  
```

d) Given the data and the prior from b): What is the probability that there is more than 80 precent water on earth? Use samples from the posterior as in in c) but use at least 10000 samples to estimate that number.

```{r, echo=lsg, eval=lsg, fig.height=4}
  sm = sample(ps, 10000, prob = post_b, replace=TRUE)
  mean(sm > 0.8)
```
