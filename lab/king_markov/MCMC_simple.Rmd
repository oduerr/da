...
a) Change the following "Metropolis King of the Archipelago" algorithm (code from https://oduerr.github.io/anim/mcmc_mh.html) so that the islands are visited proportional to the number of the island squared. Visualize the resulting chain (first 200 steps) and the distribution. 
...
```
b) For the Metropolis Algorithm (continuous case) given in https://oduerr.github.io/anim/mcmc_mh.html change the proposal distribution so that larger jumps are more likely. What do you observe? 
...
for (t in 1:Steps) {
  theta_star = rnorm(1, theta, 3)  # Propose a new value
