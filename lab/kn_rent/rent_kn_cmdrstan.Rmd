...
```
For the individual cities there is less spread. But some of them only have a few or even a single data points. Therefore full pooling does not work (for a single data points) or it's better to include information from the other cities (two or more data points). This is the case when and hierarchical models come to shine.
