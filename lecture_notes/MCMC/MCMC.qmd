...
format: 
  html:
    ...
    code-tools: true
    includes:
      in-header: ~/.quarto-headers/oduerr_ln_header.html
  pdf:
    toc: true
    toc-title: "Table of Contents"
    includes:
      in-header: ~/.quarto-headers/oduerr_ln_header.tex
  gfm: 
    toc: true
    toc-title: "Table of Contents"
    includes:
      before-body: ~/.quarto-headers/oduerr_ln_header.md
callout-exercise:
  type: exercise
  icon: "pencil-alt"
  color: "#007bff"
...
We have $x_1 = r \cos(\varphi)$ and $x_2 = r \sin(\varphi)$, so we get the Jacobian Determinant as:
...
The following code snippet shows how to sample from the target distribution in unrestricted space $x_1$ and $x_2$ and how to correctly apply the Jacobian Determinant.

```{r dount_x1x2, echo=TRUE, eval=TRUE, collapse=TRUE}
# Initial values
set.seed(123)
Steps = 10000
plot_max = 500
