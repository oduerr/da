...
[![The Histogram (actually density estimates) for of the two chains after 250 steps](2chains-marginal.png){#fig-two-chain-dist fig-align="center"}](https://github.com/oduerr/da/blob/master/lecture_notes/MCMC/SimpleMCMC.R)
...
![Traceplot of the parameter sigma inagainst the steps. While the Metropolis Algorithm provides valid samples, the samples are highly correlated compared to the more advanced Stan algorithm](traceplot_stan_vs_mh.png){#fig-traceplot fig-align="center"}
...
![Trace Plot of Samples from a Bimodal Distribution, all chains sample the repective mode quite successfully but stay too long in a mode before jumping to the next.](bimodal.png){#fig-bimodal. fig-align="center"}
...
$$ {#eq-mh-acceptance} Which is the same as in the discrete case! However, there might be pitfalls when changing to the continous case, don't just replace the probabilities $P_i$ with densities $p(\theta)$! This is for example the case in the following.
