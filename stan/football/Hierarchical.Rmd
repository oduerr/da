...
$$Averaged over all teams in the league, the attack abilities sum up to zero (at least for a huge league). The same is true for the defense abilities. Note that this is a desired property since we other wise would have to enforce that the abilities sum up to zero.

We can sample from this MVN very elegantly using via

$$
 \begin{bmatrix}att_j \\ def_j \end{bmatrix} = \begin{bmatrix} \sigma_{\tt{att}} & 0 \\ 0 & \sigma_{\tt{def}} \end{bmatrix} \cdot L' \cdot Z
$$

Where $Z$ is a 2 x J matrix with elements from $N(0,1)$ and $L'$ the transposed Cholesky decompostion matrix $L$ of the **correlation** matrix. This is great, we can give better priors for the **correlation** and **spread** compared to the covariance matrix.

### Checking the Cholesky Decomposition

Let's check this out first before we continue. Say $\sigma_{\tt att}$=1.0 and $\sigma_{\tt def}=2.0$ and the correlation is $cor({\tt att}, {\tt def})=0.8$ .
...
samples = S %*% t(L) %*% Z
plot(samples[1,], samples[2,], xlab='att', ylab='def', main='Attack and defense capabilities')
cor(t(samples))
sd(samples[1,]) #sd(samples[1,]) 
sd(samples[2,]) #sd(samples[2,]) 
```

### Coding the strength of the teams

The matrix Z is 2 times number of teams. It is ideal to code the ability of a team. At the beginning all elements are for attack and defense are standard normal $N(0,1)$ and this is then updated during the course of the games.
...
```{r}
str(rstan::extract(hfit))
L_us = rstan::extract(hfit)$L_u
```
